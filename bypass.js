(function () {
  ...
  (async function () {
    ...
    // ── Inject CSS Kustom untuk Efek Neon Biru ────────────────────────────────
    ...
    // ── Interactive Matrix Canvas Background (BIRU) ──────────────────────────
    ...
    const matrixChars = "01ABCDEFGHIJKLMNOPQRSTUVWXYZŦҖӨሃዐ".split("");
    ...
    function drawMatrix() {
      ...
      // Warna matrix dinamis berbasis biru neon
      if (matrixState === "LOGIN") {
        ctx.fillStyle = "#00d4ff"; // Biru Neon Normal
      } else if (matrixState === "OVERLOAD") {
        ctx.fillStyle = "#a855f7"; // Ungu Neon (untuk transisi)
      } else {
        ctx.fillStyle = "#00f0ff"; // Cyan Terang (Mode Bypass)
      }
    ...
    authBox.innerHTML = `
      <button id="vanz-music-btn" style="
        ...
        border:1px solid rgba(0,212,255,0.3);color:#00d4ff;border-radius:4px;
        width:32px;height:24px;cursor:pointer;font-size:10px;font-family:inherit;">MUTE</button>

      <h3 class="matrix-title-anim" style="margin:10px 0 2px 0;color:#00d4ff;font-size:24px;letter-spacing:3px;font-weight:900;text-transform:uppercase;">
        ${titleName}
      </h3>
      ...
      <input type="text" id="vanz-key-input" class="matrix-input-glow" placeholder="[ ENTER ACCESS KEY ]" style="
        width:100%;padding:14px;margin-bottom:18px;border:1px solid rgba(0,212,255,0.3);
        border-radius:6px;background:rgba(0,0,0,0.85);color:#00d4ff;text-align:center;
      ...
      <button id="vanz-login-btn" style="
        width:100%;background:#00d4ff;color:#000;border:none;padding:14px;
        border-radius:6px;font-weight:900;cursor:pointer;font-size:12px;
        letter-spacing:2px;margin-bottom:12px;font-family:inherit;
        box-shadow:0 0 15px rgba(0,212,255,0.3);transition:all 0.2s ease;">
        CONNECT_TO_HOST
      </button>
        ...
        <div style="position:relative; z-index:10; text-align:center;">
          <div style="position:relative; width:260px; height:260px; margin:0 auto; display:flex; align-items:center; justify-content:center;">
            ...
            <div style="position:absolute; top:50%; left:50%; width:230px; height:230px; border-radius:50%;
                        background:conic-gradient(transparent 0deg, #00d4ff 180deg, transparent 360deg);
            ...
            <div style="position:absolute; top:50%; left:50%; width:215px; height:215px; border-radius:50%;
                        border:2px dashed rgba(0, 212, 255, 0.3); animation:spin-counter 8s linear infinite;"></div>
            ...
            <svg width="250" height="250" style="transform:rotate(-90deg); position:relative;">
              ...
              <circle id="progress" cx="125" cy="125" r="95" fill="none" stroke="#00d4ff" stroke-width="8"
                      stroke-dasharray="${DASH_TOTAL}" stroke-dashoffset="${DASH_TOTAL}" stroke-linecap="round"
                      style="filter:drop-shadow(0 0 10px #00d4ff); transition:stroke-dashoffset 1s linear;"></circle>
            </svg>

            <div id="countdown-text" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
                        font-size:58px; font-weight:900; color:#fff; text-shadow:0 0 25px #00d4ff;">${countdownSeconds}</div>
          </div>
          <p id="bypass-hud-status" style="margin-top:35px; color:#00d4ff; font-size:12px; font-weight:bold; letter-spacing:4px;
                     text-shadow:0 0 10px #00d4ff; text-transform:uppercase;">EXTRACTING_TOKEN_STREAM...</p>
        </div>
      `;
    ...
    loginBtn.addEventListener("click", () => {
      ...
      if (isValid) {
        ...
        setTimeout(() => {
          ...
          authBox.innerHTML = `
            <h3 style="margin:5px 0 2px 0;color:#a855f7;font-size:20px;letter-spacing:2px;font-weight:900;text-shadow:0 0 10px #a855f7;">
              SELECT_METHOD
            </h3>
            ...
            <button id="vanz-btn-fast" class="vanz-mode-btn" style="border-color:#00d4ff;color:#00d4ff;background:rgba(0,212,255,0.03);">FAST MODE (RISIKO BAN)</button>
            <button id="vanz-btn-secure" class="vanz-mode-btn" style="border-color:#0088ff;color:#0088ff;background:rgba(0,136,255,0.03);">SECURE MODE (SEDANG)</button>
            <button id="vanz-btn-safe" class="vanz-mode-btn" style="border-color:#0044ff;color:#0044ff;background:rgba(0,68,255,0.03);">SAFE MODE (PALING AMAN)</button>
          `;
